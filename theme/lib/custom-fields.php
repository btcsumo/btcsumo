...
function setup() {
  ...
  // Save post meta from the Custom Feeds.
  add_action( 'save_post', __NAMESPACE__ . '\\feeds_mb_save', 10, 2 );
}
...
function feeds_mb_cb( $post ) {
  ...
  wp_nonce_field( basename( __FILE__ ), 'feeds-mb-nonce' ); ?>
 ...
 */
function feeds_mb_save( $post_id, $post ) {

  // Verify the nonce before proceeding.
  if ( ! isset( $_POST['feeds-mb-nonce'] ) || ! wp_verify_nonce( $_POST['feeds-mb-nonce'], basename( __FILE__ ) ) ) {
    return $post_id;
  }
